#include <xml_sax_handler_callbacks.h>
...
start_document(void *ctx)
{
	...
	if (ctxt->standalone != -1) {  /* -1 means there was no declaration */
	    ...
	    switch (ctxt->standalone) {
	      case 0: standalone = NOKOGIRI_STR_NEW2("no"); break;
	      case 1: standalone = NOKOGIRI_STR_NEW2("yes"); break;
	    }
...
start_element(void *ctx, const xmlChar *name, const xmlChar **atts)
{
	...
	while ((attr = atts[i]) != NULL) {
	    const xmlChar * val = atts[i+1];
            ...
            rb_ary_push(attributes, rb_ary_new3(2, NOKOGIRI_STR_NEW2(attr),
			value));
            i+=2;
	}
    }

    rb_funcall(doc, id_start_element, 2, NOKOGIRI_STR_NEW2(name), attributes);
}
...
static
VALUE attributes_as_list(int nb_attributes, const xmlChar ** attributes)
{
...
static void
start_element_ns(void *ctx, const xmlChar *localname, const xmlChar *prefix,
		 ...
		 int nb_defaulted, const xmlChar **attributes)
{
	...
	for (i = 0; i < nb_namespaces * 2; i += 2) {
	    ary = rb_ary_new3((long)2, RBSTR_OR_QNIL(namespaces[i + 0]),
			      RBSTR_OR_QNIL(namespaces[i + 1]));
	    rb_ary_push(ns_list, ary);
	}
    }

    rb_funcall(doc, id_start_element_namespace, 5, NOKOGIRI_STR_NEW2(localname),
	       attribute_list, RBSTR_OR_QNIL(prefix), RBSTR_OR_QNIL(uri),
	       ns_list);
}
 ...
 */
static void
end_element_ns(void *ctx, const xmlChar *localname, const xmlChar *prefix,
	       const xmlChar *uri)
{
    xmlParserCtxtPtr ctxt = (xmlParserCtxtPtr)ctx;
...
void
init_xml_sax_handler_callbacks()
{
